...
// declare .txt file to store data
//std::ofstream datafile ("DepthData.txt");
...
/// [loop start]
  while(!protonect_shutdown && (framemax == (size_t)-1 || framecount < framemax))
  {
    ...
    unsigned char *depth_data = depth->data;
    uint32_t timestamp = depth->timestamp;
    size_t numbytes = depth->bytes_per_pixel;
    uint32_t sequence = depth->sequence;
    size_t width = depth->width;
    size_t height = depth->height;
    
    size_t size = width*height*numbytes;
    size_t elementBytes = 1;
      
    std::string frameNum = std::to_string(sequence);
    std::string filename = "DepthData_" + frameNum + ".bin";
      
    FILE * pFile;
    const char *newfilename = filename.c_str();
    pFile = fopen (newfilename,"wb");
    fwrite(depth_data,elementBytes,size,pFile);
    fclose(pFile);
      
    unsigned char *rgb_data = rgb->data;
    uint32_t rgb_timestamp = rgb->timestamp;
    size_t rgb_bytes = rgb->bytes_per_pixel;
    uint32_t rgb_sequence = rgb->sequence;
    size_t rgb_width = rgb->width;
    size_t rgb_height = rgb->height;
      
    size_t rgb_size = rgb_width*rgb_height*rgb_bytes;
      
    std::string rgb_frameNum = std::to_string(rgb_sequence);
    std::string rgb_filename = "RGBData_" + rgb_frameNum + ".bin";
    
    FILE * rgbFile;
    const char *rgbfilename = rgb_filename.c_str();
    rgbFile = fopen (rgbfilename,"wb");
    fwrite(rgb_data,elementBytes,rgb_size,rgbFile);
    fclose(rgbFile);
      
    
// std::cout << (int) mydata[0] << std::endl;
...
//    }
    ...
    if (enable_depth)
    {
      ...
      //std::cout << "Framecount: " << framecount << " frames." << std::endl;
